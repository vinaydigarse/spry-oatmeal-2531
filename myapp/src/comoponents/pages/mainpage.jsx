...
async function FetchRandom(p){
  return  fetch(`http://localhost:3000/random?_page=${p.page}&_limit=16`).then((res)=>{
   return res.json()
  })
 }
...
return fetch(`http://localhost:3000/random?_sort=price&order=desc`).then((res)=>res.json())
  }
...
async function mobiledata(){
  return fetch(`http://localhost:3000/random?category=Mobile`).then((res)=>res.json())
    }
